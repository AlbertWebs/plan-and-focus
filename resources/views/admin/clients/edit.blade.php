...
<div class="max-w-4xl">
    ...
    <form action="{{ route('admin.clients.update', $client) }}" method="POST" class="bg-white shadow rounded-lg p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="logo" id="logo" value="{{ old('logo', $client->logo) }}" required
                ...
                @if($client->logo)
                    <div class="mt-2">
                        <img src="{{ asset('uploads/clients/' . $client->logo) }}" alt="{{ $client->name }}" class="h-16 w-auto object-contain" onerror="this.style.display='none'">
                    </div>
                @endif
                @error('logo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
